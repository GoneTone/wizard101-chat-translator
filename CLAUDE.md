...
- **註解節制使用**：預設不寫實作層 `#` 註解；寫了就要有資訊增益。可寫的情境：(1) **WHY 不顯而易見** —— 隱藏限制、微妙 invariant、bug workaround、會讓讀者意外的行為；(2) **結構/段落導引** —— 在較長函式內標出段落意圖。判準：拿掉註解後讀者是否需要多花時間理解？需要 → 留，不需要 → 刪。**預設一行**，只寫 WHY，不敘述做了什麼；真的講不清才用到兩三行，三行是上限不是目標。反例：純粹重述下一行、檔頭路徑 banner、被註解掉的舊程式碼（直接刪）、變更歷史與設計取捨的長篇論述（那些放 spec／設計文件，不進程式碼）。
- **模組與公開函式寫 docstring**：模組頂端與公開的 function／class 寫一段 `"""..."""` 說明其作用（沿用現有風格，如 `translation/translator.py`／`reader/mem_reader.py`）；簽名已自明的極簡函式可略。**長度：一行摘要**；只有呼叫者必須知道的非顯而易見限制才多加一個短段落（幾行內）。測試的 docstring 一行即可。
...
- **合併 PR 保留歷史**：PR 一律以 merge commit 合併（`gh pr merge --merge`，或 GitHub 上選 Create a merge commit），**不 squash、不 rebase** —— 每個 task 的 commit 與審查修正都要留在 `master` 的歷史裡可追。
...
- **優先用 `uv`**：本專案以 uv 管理環境（`pyproject.toml` + `uv.lock`）。相依裝好用 `uv sync`；執行用 `uv run run.py`（或 `uv run python -m src.main`）；lint 用 `uv run ruff check src tests tools`；測試用 `uv run pytest`。翻譯設定與 API 金鑰在 `config.json`（gitignore、不進版控）。

## 提交前品質檢查

執行 `git commit` 前：

1. **Lint**：`uv run ruff check src tests tools` — 必須零錯誤（未用的 import、未定義名稱、import 排序等；規則見 `pyproject.toml`）。
