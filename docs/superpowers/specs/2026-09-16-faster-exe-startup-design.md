...
`src/splash.py` 的診斷訊息沒辦法在失敗當下直接記 log：`run.py` 會在
`redirect_output()` 之前就呼叫 `update()`，而 windowed exe 在輸出導向之前
`stderr` 是 `None`（`log()` 會直接 return），那時寫出去的行會憑空消失 —— 正好是
打包版這個最需要診斷的情境。

因此失敗訊息先**緩衝**在模組級的清單裡；`main()` 在 `redirect_output()` 之後、印出
`[splash] startup screen ...` 那行狀態 log 之前，呼叫一次 `drain_logs()` 把累積的
診斷一次補寫出去，並把緩衝切換掉 —— 之後 `update()`／`close()` 若再失敗，改成直接
呼叫 `log()`，不再進緩衝。`drain_logs()` 重複呼叫安全，第二次起是無操作。

`had_failure()` 回傳「這次執行期間 `update()` 或 `close()` 是否曾經失敗過」，讓
`main()` 印狀態 log 時能一併揭露那件事，不必自己重複解析訊息內容 ——
`is_available()` 只說明 bootloader 有沒有帶 splash，兩者分開才不會把「其實已經失敗
過」誤報成「看起來正常」。

`close()` 之後的 `update()` 直接忽略（不轉給 `pyi_splash`，也不記 log）—— 首次執行
精靈那條路徑就會這樣走，真的送出去只會換來一行誤導人的失敗 log。
...
## 事後修訂：打包後實機驗證（Task 7，2026-09-16）

對照「實機驗證（打包後）」一節列出的五項，記錄使用者在真機上實際跑出來的結果。
只記已確認的事實，不做延伸推論。

- **已知風險一（焦點）／已知風險二（圖示）**：都沒有發生，使用者回報「正常」。
  「已知風險」一節事先講好的圖示補救措施（`splash.close()` 之後立刻重呼一次
  `apply_window_icon(root)`）因此沒有套用 —— 該節本來就說了只有實測看到圖示跑掉
  才動手補這一行。
- **第二實例（走 mutex 路徑，Task 4）**：`app.log` 顯示 10:44:29、10:44:33、
  10:44:59、10:45:03 這幾個 session 都在 mutex 檢查那關結束，只印出版本那一行就
  退出，通過。
- **進度條**：使用者確認中段的跳動是平滑的約 0.2 秒緩動，且（round 3 修完之後）
  進度條會在視窗出現「之前」就先到滿格；splash 關閉時沒有任何雜訊或跳出的錯誤
  視窗。
- **框選翻譯（Task 2 排除 ffmpeg／AVIF 無害）**：使用者回報正常。
- **啟動時間（`app.log`）**：`[app] version=` 到 `[app] running` 三個 session 分別
  是 0.497 秒、0.488 秒、0.522 秒（改動前是 0.467～0.481 秒）—— 這段本來就設計成
  不變，省下的時間全部發生在這行 log 之前，數字對得上預期。`import src.main` 開發
  模式下實測 0.282 秒（改動前 0.99 秒）。
- **封存內容**：未壓縮 245.5 MB（改動前 284.2 MB），exe 114.1 MB（改動前
  131.2 MB），沒有殘留 ffmpeg／`_avif`，`tcl86t.dll` 只出現一份。
- **Task 9 攔截第二次啟動**：`app.log` 12:15～12:17 這段顯示
  `[instance] watch enabled ... own_pid=77136 parent_pid=70832
  mei_name='_MEI000114b02'` —— 自我檢查成立（70832 = 0x114b0，尾碼那個固定字元是
  `2`）。四次兄弟啟動都各自完整走完 `detected → terminated → cleaned up`，耗時約
  115ms（例如 pid 62320 目錄 `_MEI0000f3702`；pid 61772 目錄 `_MEI0000f14c2`；
  pid 77384 目錄 `_MEI00012e482`）。事後 `%TEMP%` 底下沒有殘留任何 `_MEI*` 目錄。
- **尚未驗證**：兄弟以系統管理員身分執行時的行為（預期 `OpenProcess` 拒絕存取、
  退回既有的 mutex 檢查）；真實的 `ReadDirectoryChangesW` 緩衝區溢位情境。這兩項
  這次都沒有機會實測到，記在這裡以免被忘記。
...
### Fix round 2：預留 Python 階段的頂端、緩動中段的跳動

使用者實跑打包版後回報兩個現象：(1) 進度條中段會跳一大截 —— 解壓順序裡
`cv2.pyd` 前面 1000 多個小檔案只佔 7.6% 的位元組數，`cv2.pyd` 本身佔 37.6%，單一
事件把進度條從 7.6% 推到 45.2%；(2) 進度條解壓完就滿格，接著卻還要再等約 0.78
秒（`import src.main` 約 0.31 秒 ＋ `[app] version=` 到 `[app] running` 實測
0.47～0.48 秒）畫面才真的可用 —— 這段時間進度條停在滿格不動，看起來像當機。

改法 A：解壓只推進到 80%，剩下的 20% 交給 `run.py`／`src/main.py` 已經會送的兩個
階段訊息：`PHASE_LOADING`（`"Loading components..."`）推到 90%、`PHASE_STARTING`
（`"Starting..."`）推到 97%。`PHASE_LOADING`／`PHASE_STARTING` 改成定義在
`src/splash.py`（執行期真正呼叫 `splash.update()` 的地方），`tools/splash_progress.py`
在 build 時原封不動 import 進來、烤進 Tcl 的完全比對，不在 build 腳本裡另外複製
一份字面值 —— 這是結構耦合（三處都從同一個常數 import），不是抄一份常數；字面值
不同步只會讓進度條卡住，不會有任何測試變紅，除非兩邊都盯著同一個常數。

改法 B：進度推進改成緩動動畫（每步移動剩餘距離的 25%、每 16ms 一步），不再瞬間
跳到目標。splash 跑在 bootloader 自己的 threaded-Tcl 直譯器裡，解壓密集時 Tcl 的
事件迴圈不保證會處理 `after` —— 若動畫只靠 `after` 排程，解壓密集時迴圈沒空跑，
畫面就會卡住不動。因此每個 trace 事件（不論是解壓回報還是階段訊息）都無條件直接
呼叫一次步進函式 `pyi_progress_step`，確保最壞情況下（`after` 完全沒機會觸發）
進度條依然每個事件都會前進一點；步進函式自己再用一個排程旗標 `_pyi_animating`
決定要不要另外排 `after` 接力，避免同時排出兩條並行的動畫鏈。

用一個真正的 `tkinter.Tcl()` 直譯器（不建視窗）驗證清單彈出、解壓上限、階段目標、
動畫邏輯，而不只是字串比對；也補了「單次直接呼叫在 `after` 從未觸發的最壞情況下
仍會前進」與「不會疊出兩條並行動畫鏈」兩個測試。

### Fix round 3：修復 after 鏈斷鏈，讓進度條真的能到滿格

使用者實跑後回報：進度條卡在 9 成左右就不動，視窗接著就跳出來。根因：round 2 的
`_pyi_animating` 旗標只在「排程當下」設成 1，`after` 觸發、重新進入
`pyi_progress_step` 時完全沒有清掉它 —— `after` 那次呼叫看到旗標已經是 1，就不再
排下一次，整條鏈只走一次 `after` 就斷了。每個 trace 事件仍然會直接呼叫一次（所以
看起來有在動），但少了 `after` 接力，兩次直接呼叫之間的間隔（例如 `PHASE_LOADING`
到 `PHASE_STARTING`）就只補得到兩步緩動：`432 -> 440.5 -> 446.9`，剛好是 480 的
93.1%，跟使用者回報的「卡在 9 成左右」對得上；用真正的 `tkinter.Tcl()` 直譯器獨立
重現，目標 300、只手動踢一次再讓事件迴圈跑，最後停在 131.25。

round 2 的動畫測試只用直接呼叫驅動 `pyi_progress_step`（一定會前進，不受旗標
影響），完全沒有驅動過 `after` 接力鏈本身，才會沒抓到 —— round 3 補的測試改用
`vwait` 真的把 Tcl 事件迴圈跑起來，讓 `after` 鏈自己接力，才踩得到這個問題；並且
把 `pyi_progress_step` 暫時還原成 round 2 的舊版旗標邏輯重跑這個新測試，確認真的
會紅（收斂到 131.25 附近就不動），驗證測試本身的落證能力。

修法 1：把「排程旗標」換成「取消並重排」 —— `pyi_progress_step` 一進來就先用存下來
的 id 取消上一次排的 `after`，再決定要不要重排下一次。不管是直接呼叫還是 `after`
觸發，任何一次進入都會先清掉舊排程再重排，既不會漏掉接力、也不會因為兩個事件間隔
很近而疊出兩條並行的鏈。

修法 2：`PHASE_STARTING` 從推到 97% 改成推到滿格（100%）。原本刻意留 3% 的空白是
誤判 —— 一個看起來永遠填不滿的進度條讀起來就是壞掉，這正是使用者這次回報的現象。
`Starting...` 送出後到 `build_app()` 完成，實機 `app.log` 測得 0.47～0.48 秒；緩動
動畫約 200ms 收斂，滿格後畫面大約還會晾著 0.27 秒才關閉 —— 比起本功能最初要解決
的 0.78 秒空窗，這個代價很小，而「短暫的滿格」比「停在任何一個未滿的百分比」更像
正常結束，不像卡住。

保險：`pyi_progress_step` 更新畫布座標那一步包了 `catch`，防的是 splash 關閉那個
極短暫的拆除窗口裡若又被排到一次、畫布已經不在的情況 —— 查得到的事實是
PyInstaller 出貨的 IPC 樣板（`splash_templates.ipc_script`）在收到 EOF（`close()`
送出 `CLOSE_CONNECTION` 之後）會同步呼叫 `chan close $channel; exit`，理論上已經
能防住一般情況下的殘留觸發；但 bootloader 收到 `exit` 之後是整個行程結束、還是
只拆掉這個 Tcl 直譯器，屬於 C 層 bootloader 的實作細節，本模組看不到原始碼、沒有
驗證，`catch` 只是這一步的保險。實機驗證顯示 splash 關閉時沒有任何雜訊或跳出的
錯誤視窗。
