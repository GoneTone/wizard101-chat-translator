...
- **改為一律本機 OCR，模型看圖整條移除**（2026-09-15 晚）：RapidOCR 到位後兩條路徑品質相當，使用者選擇單一路徑求一致 —— 截圖不再離開本機、不必偵測模型是否支援圖片；§三的看圖方向、抄寫格式、4xx／5xx 退回與「不吃圖」標記、以及後來加的強制 OCR 設定全部作廢。
- **框選框留在畫面上、可事後調整**（2026-09-16）：放開滑鼠後矩形不消失，改成一顆只比矩形大一圈的
  透明色鍵視窗（`RegionBox`）—— 框內遊戲畫面看得到也點得到，只有框線與八個把手吃滑鼠、不奪
  焦點。拖把手縮放、拖框線移動，放開時對遊戲**當下**畫面重新 `PrintWindow`、裁切、辨識翻譯
  （這時凍結畫面已過期），卡片換成等待狀態並跟到新矩形下方；點一下沒拖動視為誤觸、還原不重跑。
  框跟卡片同進退：使用者關卡片（點一下、✕，`RegionCard.on_close`）或下一次開始框選時一起收掉。
  不加「重新翻譯」按鈕。
- **沒人要的翻譯請求要真的撤銷**（2026-09-16）：調整框、重新框選、關卡片，以及輸入框翻譯中被關掉，
  過去只靠 session 編號丟結果，伺服器仍會把整段生成完、全額計費。兩個後端的請求一律改走串流
