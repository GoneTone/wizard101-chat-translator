...
6. To read text on screen (NPC dialogue, quest books, item descriptions…): press the region hotkey (`Ctrl+Shift+Space` by default, changeable in settings) or click the region button (⛶) on the overlay's title bar. The screen freezes and dims; drag a rectangle around the text and a translation card appears right below it, with the recognized text shown for comparison. The frame stays on screen: drag inside it to move it or drag an edge to resize it, and the new area is translated again. The framed text is read by built-in offline text recognition and then translated by the AI you configured. Text on the card can be highlighted and copied with `Ctrl+C` or the right-click menu; click the card or its ✕ to dismiss it.
