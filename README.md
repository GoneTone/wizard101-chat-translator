...
6. To read text on screen (NPC dialogue, quest books, item descriptions…): press the region hotkey (`Ctrl+Shift+Space` by default, changeable in settings) or click the region button (⛶) on the overlay's title bar. The screen freezes and dims; drag a rectangle around the text and a translation card appears right below it, with the text the AI read shown for comparison. Text on the card can be highlighted and copied with `Ctrl+C` or the right-click menu; click the card or its ✕ to dismiss it. If the model does not accept images, the app falls back to its built-in offline text recognition and translates the recognized text.
