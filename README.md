...
這**可能被認定違反 [Wizard101 服務條款](https://www.wizard101.com/game/termsofuse)**，
比單純讀取更具侵入性、**封號風險更高**；依該條款，KingsIsle 可基於任何理由
（或無需理由）停權，**請自行斟酌**。
...
3. 聊天出現訊息 → 疊加視窗顯示「原文 + 目標語言譯文」（最新在最下，可向上滾動看歷史；預設不自動清除）
   ...
   - 可複製：**在訊息上按住左鍵拖曳選字**，可跨多則訊息；拖到訊息區的上下緣之外會
     自動捲動，選得到畫面外的內容。再按 `Ctrl+C` 或在選取處按右鍵選「複製」即可，
     複製出來的文字**訊息與訊息之間以空行分隔**
...
| `hotkey` | 呼出輸入框的熱鍵（預設 `ctrl+space`）；只在遊戲視窗位於前景時生效，切到其他視窗不會誤觸 |
