...
6. To read text on screen (NPC dialogue, quest books, item descriptions…): press the region hotkey (`Ctrl+Shift+Space` by default, changeable in settings; the game must be in the foreground) or click the region button (⛶) on the overlay's title bar (no need to switch to the game). The screen freezes and dims like the Windows Snipping Tool; drag a rectangle around the text — the area inside the box shows at full brightness so you can aim. When you release, a translation card appears right below the selected area, listing the text the AI read in small dim type with the translation underneath, so you can check for misreads or missed lines. Text on the card can be highlighted by dragging (from the original straight into the translation) and copied with `Ctrl+C` or the right-click Copy menu. Click the card or its ✕ to dismiss it, then press the hotkey or the button again for the next area. The screenshot goes straight to the AI you configured for recognition and translation; if the model does not accept images, the app falls back to Windows' built-in text recognition (the matching Windows OCR language pack must be installed) and translates the recognized text.
