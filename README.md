...
4. To say something: open the chat input box in the game → the translation input box appears on its own, right below the game's chat box and just as wide (it follows the game window) → type your message (in any language) → Enter → the software switches back to the game and **types the translated English into the chat box character by character** → check it yourself, then press Enter to send.
