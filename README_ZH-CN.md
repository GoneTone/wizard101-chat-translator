...
4. 想发言：在游戏里打开聊天输入框 → 翻译输入框自动出现在游戏聊天输入框正下方、与它同宽（每次打开聊天输入框时重新对齐）→ 输入消息（任何语言）→ Enter → 软件切回游戏、把翻好的英文**逐字自动键入**聊天输入框 → 自己检查后按 Enter 发送。
   - 若把翻译输入框关掉了，或在设置内关闭了自动呼出（`auto_show_input`），可以按 `Ctrl+Space`（默认）叫它出来：游戏聊天输入框开着就出现在它底下，没开就出现在鼠标光标处。热键只在游戏窗口位于前台时生效，切到其他程序不会误触。
...
5. 想改设置（服务商、密钥、目标语言、热键…）时，点叠加窗口标题栏的齿轮（⚙）打开设置窗口，保存即生效，不必重开程序（只有「高级」标签页的「游戏路径」要下次启动才生效）。
...
从 Crowdin 下载的译文只会带真正翻好的字符串（`skip_untranslated_strings`），所以语言文件只有部分 key 是正常状态 —— 缺的逐键 fallback，先退英文、最后退源语言。语言要进得了菜单，得同时声明自称、且完成度达源语言的 80%（`MINIMUM_COVERAGE`）：Crowdin 会替每个目标语言产文件，空的或才起步的语言进了菜单，选下去只是一个半英半外的界面。源语言与 `en-US` 一律列入。被挡下的语言与其完成度会记在 `app.log`。
