...
4. 想发言：在游戏里打开聊天输入框 → 翻译输入框自动出现在游戏聊天输入框正下方、与它同宽（每次打开聊天输入框时重新贴齐）→ 输入消息（任何语言）→ Enter → 软件切回游戏、把翻好的英文**逐字自动键入**聊天输入框 → 自己检查后按 Enter 发送。
   ...
   - 打到一半游戏把聊天输入框关了（例如按到 Esc），翻译输入框会跟着收起，但尚未发送的文字会留着，下次呼出时还原；自己按 Esc 或 ✕ 关闭的才会清掉。
