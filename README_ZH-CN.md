...
1. 双击运行 exe：
   - **翻译服务尚未设置完成**（第一次运行、或 `config.json` 里缺模型／密钥／自建服务器网址）会弹出**首次设置向导**：第一步选界面语言，第二步选翻译服务、填密钥并测试连接，第三步选目标语言、热键与是否自动呼出翻译输入框。保存后自动进入主流程。
...
设置窗口：翻译服务、API 密钥、目标语言与快捷键，保存后立即生效、不必重启：
