...
6. 想看懂画面上的文字（NPC 对话、任务书、道具说明…）：按框选热键（默认 `Ctrl+Shift+Space`，可在设置里改）或点叠加窗口标题栏的框选按钮（⛶），画面会定格变暗，拖一个矩形框住要翻的文字，译文卡片就出现在框选区域正下方，并附上 AI 读到的原文供对照。卡片上的文字可选中，`Ctrl+C` 或右键复制；点卡片或 ✕ 关掉。模型不支持看图时会自动改用 Windows 内置的文字识别再翻译（需安装对应的 OCR 语言包）。
