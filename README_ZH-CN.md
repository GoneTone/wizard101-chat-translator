...
6. 想看懂画面上的文字（NPC 对话、任务书、道具说明…）：按框选热键（默认 `Ctrl+Shift+Space`，可在设置里改；游戏要在前台），或点叠加窗口标题栏的框选按钮（⛶，不必切到游戏）。画面会像 Windows 截图工具一样定格并变暗，用鼠标拖一个矩形框住要翻的文字，框内会恢复亮度方便对准；松开后译文卡片出现在框选区域正下方，卡片上先以暗色小字列出 AI 读到的原文、下方接译文，可以直接对照有没有读错或漏读。卡片上的文字可以拖拽选中（可从原文一路选到译文），`Ctrl+C` 或右键“复制”即可复制；点一下卡片或右上角的 ✕ 关掉，再按热键或按钮可以框下一个。截图直接交给你设置的 AI 识别并翻译；模型不支持看图时会自动改用 Windows 内置的文字识别再翻译（需要安装对应的 Windows OCR 语言包）。
...
- **画面上的文字也能翻**：按热键或标题栏按钮框选 NPC 对话、任务书、道具说明等任意区域，画面定格让你慢慢框；译文贴在框选处下方，附上 AI 读到的原文可对照，文字可选中复制
