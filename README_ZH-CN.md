...
**看画面**：聊天以外的文字 —— NPC 对话、任务书、道具说明 —— 也翻得了。按热键让画面定格，把文字框起来，译文卡片就贴在框选处旁边，上面附着识别到的原文可以对照。
...
1. 双击运行 exe：
   - **翻译服务尚未设置完成**（第一次运行、或 `config.json` 里缺模型／密钥／自建服务器网址）会弹出**首次设置向导**：第一步选界面语言，第二步创建第一组翻译服务、填密钥并测试连接，第三步选目标语言、热键与是否自动呼出翻译输入框。保存后自动进入主流程。
   - 设置完整后每次启动都直接进主流程，不会再弹向导；界面语言仍可随时在设置窗口（齿轮 ⚙）改，保存即立即生效，不必重开程序。
2. 打开 Wizard101 并**登录进游戏世界内**，叠加窗口才读得到聊天消息。
3. 聊天出现消息 → 叠加窗口显示**原文 + 目标语言译文**（最新在最下，可向上滚动看历史；默认不会依时间淡出，最多保留 200 条，超过会移除最旧；两者都可在设置的「高级」标签页调整）。
...
4. 想发言：在游戏里打开聊天输入框 → 翻译输入框自动出现在游戏聊天输入框正下方、与它同宽（每次打开聊天输入框时重新对齐）→ 输入消息（任何语言）→ Enter → 软件切回游戏、把翻好的英文**逐字自动键入**聊天输入框 → 自己检查后按 Enter 发送。
   - 若把翻译输入框关掉了，或在设置内关闭了自动呼出，可以按 `Ctrl+Space`（默认）叫它出来：游戏聊天输入框开着就出现在它底下，没开就出现在鼠标光标处。热键只在游戏窗口位于前台时生效，切到其他程序不会误触。
   - 译文超过游戏聊天输入框的 80 字上限时不会键入，翻译输入框会提示字数、留着原文让你删减或分段后重发。
   - 打字到一半游戏把聊天输入框关了，翻译输入框会跟着收起，但尚未发送的文本会留着，下次呼出时还原；自己按 Esc 或 ✕ 关闭的才会清掉。
   - 译文自动键入进游戏时丢字／太快 → 打开设置窗口（⚙），在「高级」标签页调大「键入延迟（秒）」。
   - 打字期间光标要停在游戏聊天输入框。若切到别的窗口，键入会当场停止，剩下的字不会打进那个窗口。
5. 想粘贴内容到游戏里：游戏本身不支持 `Ctrl+V`，所以游戏窗口在前台时，软件会接手 `Ctrl+V`，改成把剪贴板文本**逐字自动键入**（速度同键入延迟设置）。游戏内任何可输入的地方都适用，不限聊天输入框；游戏聊天输入框开着时换行会打成空格，避免多行内容打到一半就被发送，其他地方换行照打成 Enter。在其他窗口按 `Ctrl+V` 完全不受影响。不想要可在设置内关闭。
6. 想看懂画面上的文字（NPC 对话、任务书、道具说明…）：按框选热键（默认 `Ctrl+Shift+Space`）或点叠加窗口标题栏的框选按钮（⛶）→ 画面定格变暗 → 拖一个矩形框住文字 → 译文卡片出现在框选区域旁边。
   - 框选框会留在画面上：拖框内移动、拖边线改大小，新范围会自动重新翻译。
   - 卡片上方附识别到的原文供对照；文字可选中，`Ctrl+C` 或右键复制。点卡片或 ✕ 关掉。
   - 文字由内置的离线文字识别读出，再交给你设置的 AI 翻译；画面不会离开你的电脑。
7. 想改设置（服务商、密钥、目标语言、热键…）时，点叠加窗口标题栏的齿轮（⚙）打开设置窗口，保存即生效，不必重开程序（只有「高级」标签页的「游戏路径」要下次启动才生效）。
8. 要结束程序时，点叠加窗口标题栏右上角的 ✕（会先解除游戏挂入再退出）。
9. 每次启动会自动检查 GitHub 上有没有新版本，有的话叠加窗口会多出一条蓝色横幅（点横幅开下载页，点右侧 ✕ 关掉这次提醒）；也可以在设置窗口的「关于」标签页手动检查更新、查看项目链接与日志文件所在文件夹。

## 功能与特色

- **看得懂别人在说什么**：聊天每一句都实时翻成你的语言，原文保留在译文上方；消息一次涌进来也跟得上
- **用自己的语言发言**：打开游戏聊天输入框，翻译输入框就自动出现；打完按 Enter，英文译文自动键入聊天框，要不要发送由你决定
- **文本可以粘贴到游戏里**：游戏本身不能粘贴文本，游戏在前台时按 `Ctrl+V` 就由软件替你逐字键入剪贴板内容，任何输入栏位都适用
- **不会看错字、不会漏消息**：直接读游戏的聊天内容，不是识别画面，他人与自己的发言都涵盖
- **画面上的文字也能翻**：按热键或标题栏按钮框选 NPC 对话、任务书、道具说明等任意区域，画面定格让你慢慢框；译文贴在框选处旁边，附上识别到的原文可对照，文字可选中复制
- **要翻成什么语言你自己填**：直接填语言名称（`繁體中文（台灣）`、`日本語`、`Español`…），对方用什么语言则由 AI 自动判断
- **选你要用的 AI**：OpenAI（ChatGPT）、Anthropic（Claude），或自己搭的 OpenAI 兼容服务 —— 可以创建多组服务并随时切换，聊天收信、发话与区域翻译也能各自指定不同的一组
...
- **自动键入译文时游戏丢字**：在设置窗口（⚙）的「高级」标签页调大「键入延迟（秒）」，且打字期间不要切换窗口（游戏一离开前台键入就会停止）。

## 截图

叠加窗口浮在运行中的游戏画面上 —— 每条聊天的原文都留在译文上方：

![叠加窗口浮在游戏画面上，显示翻译后的对话](docs/images/1.png)

在游戏里打开聊天输入框，翻译输入框就会自己弹出来，用任何语言输入都可以：

![翻译输入框，里面已输入消息](docs/images/2.png)

英文译文被逐字打进游戏的聊天输入框 —— 要不要按 Enter 发送，由你决定：

![游戏的聊天输入框，里面是已打入的英文译文](docs/images/3.png)

把画面上的文字框起来 —— NPC 对话、任务书、道具说明都行 —— 译文卡片就贴在框选处旁边，上面附着识别到的原文可以对照：

![框住游戏内的一条提示，旁边是译文卡片](docs/images/4.png)

设置窗口：翻译服务、目标语言与快捷键，保存后立即生效、不必重启：

![设置窗口的基本标签页](docs/images/5.png)

## 已知限制

- 游戏聊天白名单：非白名单英文词可能被游戏自动过滤，任何翻译软件都绕不过。
- 译文不会瞬间出现：默认每 0.4 秒扫一次聊天（「高级」标签页可调），再等翻译服务响应才显示，慢多少主要看你选的服务商与模型。
...
- 依赖固定的内存特征（pattern）：游戏更新后 pattern 可能失效（挂入时报 `PatternFailed`，或 hook 挂上了却不被触发），需等支持跟进才会恢复；从源码运行时，是等接收消息用的依赖包更新后重跑 `uv sync`。两种情形叠加窗口都显示「⚠  游戏版本不兼容」横幅，与「游戏没开」分得开。
...
1. `uv sync` —— 创建 `.venv` 并依 `pyproject.toml` 装好所有依赖（含接收消息用的 wizwalker；已在 `[tool.uv.sources]` 指向有跟进最新 client pattern 的 [LaurenzNotHere fork](https://codeberg.org/LaurenzNotHere/wizwalker)，官方 PyPI 版 pattern 过旧、对不上现行 client）。
...
- `messages.log`：接收端读到的**原始**聊天内容，不做任何清理与过滤（`RAW` 为含标记的原文、含系统消息，只排除游戏自己灌进聊天控件的 `[WARN]`／`[ERRO]`／`[DBGM]` 之类的调试行；`OUT` 为实际送去翻译的行）。消息漏翻／重复翻译之类的问题请一并附上这份。
