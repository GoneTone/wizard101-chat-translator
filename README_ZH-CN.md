...
6. 想看懂画面上的文字（NPC 对话、任务书、道具说明…）：按框选热键（默认 `Ctrl+Shift+Space`，可在设置里改）或点叠加窗口标题栏的框选按钮（⛶），画面会定格变暗，拖一个矩形框住要翻的文字，译文卡片就出现在框选区域正下方，并附上识别到的原文供对照。框选框会留在画面上，拖手柄或框线调整范围就会重新翻译。框到的文字由内置的离线文字识别读出，再交给你设置的 AI 翻译。卡片上的文字可选中，`Ctrl+C` 或右键复制；点卡片或 ✕ 关掉。
