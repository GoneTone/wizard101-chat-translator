...
6. 想改设置（服务商、密钥、目标语言、热键…）时，点叠加窗口标题栏的齿轮（⚙）打开设置窗口，保存即生效，不必重开程序（只有「高级」标签页的「游戏路径」要下次启动才生效）。
7. 要结束程序时，点叠加窗口标题栏右上角的 ✕（会先解除游戏挂入再退出）。
8. 每次启动会自动检查 GitHub 上有没有新版本，有的话叠加窗口会多出一条蓝色横幅（点横幅开下载页，点右侧 ✕ 关掉这次提醒）；也可以在设置窗口的「关于」标签页手动检查更新、查看项目链接与日志文件所在文件夹。
