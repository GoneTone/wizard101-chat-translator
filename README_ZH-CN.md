...
6. 想看懂画面上的文字（NPC 对话、任务书、道具说明…）：按框选热键（默认 `Ctrl+Shift+Space`，可在设置里改）或点叠加窗口标题栏的框选按钮（⛶），画面会定格变暗，拖一个矩形框住要翻的文字，译文卡片就出现在框选区域正下方，并附上识别到的原文供对照。框选框会留在画面上，拖框内移动、拖边线改大小，就会重新翻译新范围。框到的文字由内置的离线文字识别读出，再交给你设置的 AI 翻译。卡片上的文字可选中，`Ctrl+C` 或右键复制；点卡片或 ✕ 关掉。
...
- **画面上的文字也能翻**：按热键或标题栏按钮框选 NPC 对话、任务书、道具说明等任意区域，画面定格让你慢慢框；译文贴在框选处下方，附上识别到的原文可对照，文字可选中复制
