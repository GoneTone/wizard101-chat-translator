...
def test_show_while_open_replaces_the_previous_layer(selector, root):
    selector.show(_MONITOR, _FRAME, on_select=lambda r: None, on_cancel=lambda: None)
    first = selector._win
    selector.show(_MONITOR, _FRAME, on_select=lambda r: None, on_cancel=lambda: None)
    root.update()
...
def test_backdrop_is_placed_below_the_game_frame_when_given(selector, root):
    ...
    selector.show(_MONITOR, _FRAME, on_select=lambda r: None, on_cancel=lambda: None, backdrop=backdrop)
    root.update()
...
def test_hint_window_closes_after_a_completed_drag(selector, root):
    picked = []
    selector.show(_MONITOR, _FRAME, on_select=picked.append, on_cancel=lambda: None)
    root.update()
