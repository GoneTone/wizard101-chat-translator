...
6. 想看懂畫面上的文字（NPC 對話、任務書、道具說明…）：按框選熱鍵（預設 `Ctrl+Shift+Space`，可在設定改；遊戲要在前景），或點疊加視窗標題列的框選按鈕（⛶，不必切到遊戲）。畫面會像 Windows 剪取工具一樣定格並變暗，用滑鼠拖一個矩形框住要翻的文字，框內會恢復亮度方便對準；放開後譯文卡片出現在框選區域正下方，卡片上先以暗色小字列出 AI 讀到的原文、下方接譯文，可以直接對照有沒有讀錯或漏讀。卡片上的文字可以拖曳反白（可從原文一路選到譯文），`Ctrl+C` 或按右鍵「複製」即可複製；點卡片一下或右上角的 ✕ 關掉，再按熱鍵或按鈕可以框下一個。截圖直接交給你設定的 AI 辨識並翻譯；模型不支援看圖時會自動改用 Windows 內建的文字辨識再翻譯（需要安裝對應的 Windows OCR 語言包）。
...
- **畫面上的文字也能翻**：按熱鍵或標題列按鈕框選 NPC 對話、任務書、道具說明等任意區域，畫面定格讓你慢慢框；譯文貼在框選處下方，附上 AI 讀到的原文可對照，文字可反白複製
