...
6. 想看懂畫面上的文字（NPC 對話、任務書、道具說明…）：按框選熱鍵（預設 `Ctrl+Shift+Space`，可在設定改）或點疊加視窗標題列的框選按鈕（⛶），畫面會定格變暗，拖一個矩形框住要翻的文字，譯文卡片就出現在框選區域正下方，並附上辨識到的原文供對照。框選框會留在畫面上，拖把手或框線調整範圍就會重新翻譯。框到的文字由內建的離線文字辨識讀出，再交給你設定的 AI 翻譯。卡片上的文字可反白，`Ctrl+C` 或右鍵複製；點卡片或 ✕ 關掉。
