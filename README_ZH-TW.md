...
**看畫面**：聊天以外的文字 —— NPC 對話、任務書、道具說明 —— 也翻得了。按熱鍵讓畫面定格，把文字框起來，譯文卡片就貼在框選處旁邊，上面附著辨識到的原文可以對照。
...
1. 雙擊執行 exe：
   - **翻譯服務尚未設定完成**（第一次執行、或 `config.json` 裡缺模型／金鑰／自架伺服器網址）會跳出**首次設定精靈**：第一步選介面語言，第二步建立第一組翻譯服務、填金鑰並測試連線，第三步選目標語言、熱鍵與是否自動呼出翻譯輸入框。存好設定後自動進入主流程。
...
4. 想發言：在遊戲裡開啟聊天輸入框 → 翻譯輸入框自動出現在遊戲聊天輸入框正下方、與它同寬（每次開啟聊天輸入框時重新貼齊）→ 輸入訊息（任何語言）→ Enter → 軟體切回遊戲、把翻好的英文**逐字自動鍵入**聊天輸入框 → 自己檢查後按 Enter 送出。
   - 若把翻譯輸入框關掉了，或在設定內關閉了自動呼出，可以按 `Ctrl+Space`（預設）叫它出來：遊戲聊天輸入框開著就出現在它底下，沒開就出現在滑鼠游標處。熱鍵只在遊戲視窗位於前景時生效，切到其他程式不會誤觸。
   ...
   - 打字期間游標要停在遊戲聊天輸入框。若切到別的視窗，鍵入會當場停止，剩下的字不會打進那個視窗。
5. 想貼上內容到遊戲裡：遊戲本身不支援 `Ctrl+V`，所以遊戲視窗在前景時，軟體會接手 `Ctrl+V`，改成把剪貼簿文字**逐字自動鍵入**（速度同鍵入延遲設定）。遊戲內任何可輸入的地方都適用，不限聊天輸入框；遊戲聊天輸入框開著時換行會打成空格，避免多行內容打到一半就被送出，其他地方換行照打成 Enter。在其他視窗按 `Ctrl+V` 完全不受影響。不想要可在設定內關閉。
6. 想看懂畫面上的文字（NPC 對話、任務書、道具說明…）：按框選熱鍵（預設 `Ctrl+Shift+Space`）或點疊加視窗標題列的框選按鈕（⛶）→ 畫面定格變暗 → 拖一個矩形框住文字 → 譯文卡片出現在框選區域旁邊。
   - 框選框會留在畫面上：拖框內移動、拖邊線改大小，新範圍會自動重新翻譯。
   - 卡片上方附辨識到的原文供對照；文字可反白，`Ctrl+C` 或右鍵複製。點卡片或 ✕ 關掉。
   - 文字由內建的離線文字辨識讀出，再交給你設定的 AI 翻譯；畫面不會離開你的電腦。
7. 想改設定（服務商、金鑰、目標語言、熱鍵…）時，點疊加視窗標題列的齒輪（⚙）開啟設定視窗，存檔即套用，不必重開程式（只有「進階」分頁的「遊戲路徑」要下次啟動才生效）。
8. 要結束程式時，點疊加視窗標題列右上角的 ✕（會先解除遊戲掛入再退出）。
9. 每次啟動會自動檢查 GitHub 上有沒有新版本，有的話疊加視窗會多出一條藍色橫幅（點橫幅開下載頁，點右側 ✕ 關掉這次提醒）；也可以在設定視窗的「關於」分頁手動檢查更新、查看專案連結與紀錄檔所在資料夾。

## 功能與特色

- **看得懂別人在說什麼**：聊天每一句都即時翻成你的語言，原文保留在譯文上方；訊息一次湧進來也跟得上
- **用自己的語言發言**：開啟遊戲聊天輸入框，翻譯輸入框就自動出現；打完按 Enter，英文譯文自動鍵入聊天框，要不要送出由你決定
- **文字可以貼上到遊戲裡**：遊戲本身不能貼上文字，遊戲在前景時按 `Ctrl+V` 就由軟體替你逐字鍵入剪貼簿內容，任何輸入欄位都適用
- **不會看錯字、不會漏訊息**：直接讀遊戲的聊天內容，不是辨識畫面，他人與自己的發言都涵蓋
- **畫面上的文字也能翻**：按熱鍵或標題列按鈕框選 NPC 對話、任務書、道具說明等任意區域，畫面定格讓你慢慢框；譯文貼在框選處旁邊，附上辨識到的原文可對照，文字可反白複製
- **要翻成什麼語言你自己填**：直接填語言名稱（`繁體中文（台灣）`、`日本語`、`Español`…），對方用什麼語言則由 AI 自動判斷
- **選你要用的 AI**：OpenAI（ChatGPT）、Anthropic（Claude），或自己架的 OpenAI 相容服務 —— 可以建立多組服務並隨時切換，聊天收訊、發話與區域翻譯也能各自指定不同的一組
...
- **自動鍵入譯文時遊戲漏字**：在設定視窗（⚙）的「進階」分頁調大「鍵入延遲（秒）」，且打字期間不要切換視窗（遊戲一離開前景鍵入就會停止）。
...
把畫面上的文字框起來 —— NPC 對話、任務書、道具說明都行 —— 譯文卡片就貼在框選處旁邊，上面附著辨識到的原文可以對照：

![框住遊戲內的一則提示，旁邊是譯文卡片](docs/images/4.png)

設定視窗：翻譯服務、目標語言與快捷鍵，儲存後立即生效、不必重啟：

![設定視窗的基本分頁](docs/images/5.png)
