...
6. 想改設定（服務商、金鑰、目標語言、熱鍵…）時，點疊加視窗標題列的齒輪（⚙）開啟設定視窗，存檔即套用，不必重開程式（只有「進階」分頁的「遊戲路徑」要下次啟動才生效）。
7. 要結束程式時，點疊加視窗標題列右上角的 ✕（會先解除遊戲掛入再退出）。
8. 每次啟動會自動檢查 GitHub 上有沒有新版本，有的話疊加視窗會多出一條藍色橫幅（點橫幅開下載頁，點右側 ✕ 關掉這次提醒）；也可以在設定視窗的「關於」分頁手動檢查更新、查看專案連結與紀錄檔所在資料夾。
