...
def largest_group(marks: list[int], gap: int = _GROUP_GAP) -> tuple[int, int] | None:
    """把已排序的標記位址依間隔分群,回傳「標記最多那群」的 (起, 迄)。空則 None。
    聊天在記憶體有多份近乎完整的副本;最大群 = 最完整的聊天文件,其結尾即最新訊息。"""
    if not marks:
        return None
    best = (marks[0], marks[0])
    best_n = 1
    start = prev = marks[0]
    n = 1
    for a in marks[1:]:
...
def read_ordered_tail(process_name: str = PROCESS_NAME, tail: int = 80) -> list[str]:
    """讀取「最完整聊天文件」的有序尾段(最後 tail 行,依時間順序)。
    ...
    try:
        ...
        marks.sort()
        group = largest_group(marks)
        if group is None:
            return []
        start, end = group
        blob = _read(h, start, min(end - start + 2000, 8_000_000))
        lines = extract_lines(blob)  # 依位址(=時間)順序、群內去重
